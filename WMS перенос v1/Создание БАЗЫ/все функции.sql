...
DROP FUNCTION RABAEV.RRL_GET_ST_IS_SCANNED;
...
DROP FUNCTION RABAEV.RRL_PALLET_WEIGHT_SET;
...
DROP FUNCTION RABAEV.RRL_SET_TT_LOAD_TASK;
...
update RRL_TRANSPORT_TASK set   DOCK_REZERV_TIME_FROM = null ,  DOCK_REZERV_TIME_TO = null where ID=TT;
...
for WW_wares_id1  in  WW_wares_id loop -- ЦИКЛ ПО ВСЕМ СКЛАДАМ, ПАЛЛЕТЫ КОТОРЫХ УЧАВСТВУЮТ В МАРШРУТЕ , ПОРЯДОК СКЛАДОВ БЕРЕТСЯ ИЗ ПОЛЯ ORD2 !!! 
    ...
    DBMS_OUTPUT.put_line( 'после' );
            ...
            count1:=0;
            exit1:=0;
            time2:=time1; -- Время, на которое планируется док. 
