...
-- Created by rrabaev on 12.06.2011, 15:24:59 --
------------------------------------------------
...
begin 
    delete from RRL_TT_LOAD_TASK where DOCK = ZONE1 and LOAD_FROM  <= time1 and
    LOAD_TO >= time3;
exception
    when no_data_found then null;
    when others then null;
...
insert into RABAEV.RRL_TT_LOAD_TASK (  TT_ID  ,DOCK ,LOAD_FROM , LOAD_TO , WARE_ID ) values 
    ( TT , ZONE1 , time1 , time3 , MAIN_WARE_ID );
...
if( type_w1=0 ) then  -- ТИП товара = 0     
    if(COUNT_SHT_IN_KOR1= 0) or ( COUNT_SHT_IN_KOR1 is null ) then
        return 'Для Артикула '+articul1+' не указана вложенность. Укажите вложенность.';
    end if;
...
if( type_w1=1 ) then    -- ТИП товара = 1  
     ...
     if(CURRENT_MOD_ID2   = 0) or ( CURRENT_MOD_ID2 is null ) then
            return 'Для артикула '+articul1+' не указана фасовка. Укажите фасовку.';
     end if;
...
if( type_w1=2 ) then    -- ТИП товара = 2  
     ...
     if(PRIHOD_PALLET_UID2   = '') or ( PRIHOD_PALLET_UID2 is null ) then
            return 'Для артикула '+articul1+' не указана партия. Укажите партию.';
     end if;
...
begin
  ...
  insert into  RABAEV.RRL_ORDERS (CREATE_DATE ,WARE_ID ,ADDR ,STATE ,ORDDATE , USER_ID) 
  values(CREATE_DATE1,WARE_ID1 ,ADDR1 ,STATE1  , ORDDATE1, user_id1);
...
-- КУРСОР СУММАРНЫЙ ЛИСТ .       
  ...
  select ARTICUL , Q , compl.show_sq(ARTICUL , ware_id2) SEQ , 
   ...
   from (
       select RS.ARTICUL , sum( RS.QUANTITY ) Q from RABAEV.RRL_SBORKA_PALLETS PAL , 
       RABAEV.RRL_SBORKA_PALLET_ROWS RS
       where PAL.SUMMARY_TASK_ID = summary_task_id1 and
       PAL.PALLET_UID=RS.PALLET_UID and RS.QUANTITY >0 group by RS.ARTICUL
   ) )
    ...
    for ss in sss loop -- По всем строкам ссумарного сборочного листа (ССЛ)
        -- DBMS_OUTPUT.put_line(  ss.ARTICUL ); 
         ...
         if(skolko_nado>0) then
                  ...
                  'WR1' ,  ost1.UID_POLETA  , ost1.cell , 'SBOR' , count2 , 
                  --null , null ,null , null 
                  null , 0 , summary_task_id1);
