      ...
      public  void Console_WriteLine(string str)
        ...
        private void AcceptConnections()
        {
            ...
            switch (F_Request1.function_name)
            {
                ...
                case "VP2":
                    string PALLET_UID_1 = F_Request1.get_value("PALLET_UID");
                    string ARTICUL_1 = F_Request1.get_value("ARTICUL");
                    string COLV_1 = F_Request1.get_value("COLV");

                    Console.WriteLine("VP2" + PALLET_UID_1);
                break;
                   

                #region СООБЩЕНИЕ_О_ПРИЕМКЕ
                case "PRIEMKA_PASSED":
                Console_WriteLine("PRIEMKA_PASSED!");
                // Сборка прошла проверку
                USSCC = "";
                    ...
                    foreach (funct f5 in F_Request)
                    {
                        if (f5.function_name == "VP2")
                        {
                                ...
                                try
                                {
                                    ...
                                    string ORDER_ID = f5.strToIntMap["PALLET_UID"].Replace("N_", "");
                                    string ARTICUL = f5.strToIntMap["ARTICUL"];
                                    string COLV = f5.strToIntMap["COLV"];
                                    strSQL = " update   RABAEV.RRL_PRIHOD_NAKLAD_ROWS  set COUNT1 = " + COLV + " where  ( ORDID='" + ORDER_ID
                                        + "' ) and ( ARTICUL = '" + ARTICUL + "' ) ";
...
" and SUBSTR(UE_ADRUMS,  LENGTH(UE_ADRUMS)-1 ,1  ) in ('1','2') ";
                ...
                case "GET_PRIH_ITEMS": // GET_ORDER_ITEMS
                    ...
                    strSQL = "select '' as dumm1 , "+ 
                    " ART.ACTICUL ,  "+ 
                    " RS.COUNT1 ,  "+ 
                    " ART.NAME ,  "+ 
                    " ART.COUNT_SHT_IN_KOR ,  "+ 
                    " ART.BARCODE_SHT ,  "+ 
                    " ART.BARCODE_KOR  , "+
                    " ART.COUNT_SHT_IN_BL ,  ART.BARCODE_BL  " + 
                    "  from RABAEV.RRL_PRIHOD_NAKLAD NAK , RABAEV.RRL_PRIHOD_NAKLAD_ROWS RS , RABAEV.RRL_ARTICULS ART  "+ 
                    "  where  "+ 
                    " NAK.ID = RS.ORDID  and "+ 
                    " RS.ARTICUL = ART.ACTICUL and "+ 
                    " NAK.ID = '"+OrderNumber+"' order by RS.ID ";
                    ...
                    lines_count_must_be = 0;
                        ...
                        while (ora_reader.Read())
                        {
                            ...
                            r = new funct();
                            r.function_name = "LOT_LINE";
                            r.add_value("ПП",  pos.ToString() );
                            //r.add_value("USSCCD", ora_reader.GetString(0));
                            r.add_value("УИД", ora_reader.GetString(1)); // артикул 
                            r.add_value("КОЛ", Convert.ToString(ora_reader.GetInt64(2))); // количество прихода
                            r.add_value("НАИМ", ora_reader.GetString(3));  /// имя товара
                            r.add_value("СЛУЖ_АДР", pos.ToString()); // Ячейка отбора

                            r.add_value("СЛУЖ_ШТВБЛ", ora_reader.GetInt64(7).ToString() );
                            r.add_value("СЛУЖ_БЛВКОР", ora_reader.GetInt64(4).ToString()); // Штук в коробке

                            r.add_value("СЛУЖ_ШК_ШТУКИ", ora_reader.GetOracleString(5)); //
                            r.add_value("СЛУЖ_ШК_БЛОКА", ora_reader.GetOracleString(8));
                            r.add_value("СЛУЖ_ШК_КОРОБ", ora_reader.GetOracleString(6)); //
                            //sfera_ean.EAN_SHT , sfera_ean.EAN_BL , sfera_ean.EAN_KOR 
                            summary = summary + r.encode();
                            sql_product = sql_product + ", " + ora_reader.GetString(1);
                        }
                        ...
                        #region ВЫБОРКА_ДОПОЛНИТЕЛЬНЫХ_ШТРИХ_КОДОВ - РАЗКОММЕНТИРОВАТЬ КОГДА ПОНАДОБЯТСЯ
...
" ORDER BY ТранспортныеЗаданияСтроки.ДатаТЗ, ТранспортныеЗадания.Экспедитор; ";
    ...
    class Program
    {
        ...
        if (ware_id == 3)
        {
            IP_listen = "192.168.0.100";
            IP_listen = "192.168.0.100"; PORT_p = 20000;
           // IP_listen = "192.168.208.205";
