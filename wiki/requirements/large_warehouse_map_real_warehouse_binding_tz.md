...
- для дробного отбора адресуются дочерние `PICK_FACE_SLOT` через маску дробления, а не физический pick-face address полноразмерной ячейки;
...
Дробная ячейка
  Отбор (flyout / боковое подменю)
    ...
    Пресет: 3/3
    Пресет: 3/4
    Пресет: V/G... (dialog asks vertical V and horizontal G)
  Хранение (flyout / боковое подменю)
...
- default split preset for pick-face: `2/1`;
...
- context menu:
  - `Отбор -> Полноразмерный отбор`;
  - `Отбор -> Дробление -> 2/1, 2/2, 2/3, 3/1, 3/2, 3/3, 3/4`;
  - `Отбор -> Дробление -> V/G...`, где диалог спрашивает вертикальный размер `V` и горизонтальный размер `G`, затем создает `V * G` child slots;
  - `Хранение -> Полноразмерное хранение`;
  - `Хранение -> Дробление -> пресет`;
  - `Настроить вместимость storage slots`.
...
- назначение `Дробные ячейки отбора` без ручного выбора пресета создает `2` pick slots по пресету `2/1`;
...
- пункты верхнего уровня context menu должны открывать боковое подменю по hover и по click, чтобы оператор понимал, что клик по `Отбор` не является пустым действием;
- если API draft еще не создан, команда создания дробных slots должна создать draft автоматически и затем выполнить выбранное дробление;
