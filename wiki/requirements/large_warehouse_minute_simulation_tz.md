...
- ворота `G01..G25`;
- зоны накопления перед воротами, куда выставляется собранный товар перед отгрузкой;
- ячейки отбора;
- зоны хранения;
- зона отгрузки;
...
- явные подписи проходов прямо на полу карты: фронтальный проезд, пожарный проход и задний обход;
- подсветка коллизий;
...
При клике на ресурс или паллету показывать статус, задачу, волну, клиента, SKU, откуда/куда, плановое и фактическое время, причину задержки и накопленные потерянные минуты. Если ресурс замедлен, заблокирован или ожидает, карточка должна показывать причину, соседний влияющий ресурс, проблемную задачу пополнения или адрес, из-за которого возник риск.

При наведении на не полностью заполненную ячейку отбора показывать:

- текущий остаток и процент заполнения;
- открытые задачи пополнения на эту ячейку;
- какие задачи уже являются hard/driver-facing, то есть выпущены конкретному водителю РТК;
- просрочку задач;
- обоснование просрочки или ожидания release-policy.
...
Зона накопления перед воротами должна отображать фактические staged-паллеты:

- емкость визуализации не менее 33 паллет;
- паллеты ставятся в 2 ряда, до 16 паллет по длине;
- паллеты, привезенные РТК со стеллажа, отображаются как solid pallet;
- паллеты, собранные комплектовщиком из коробок, отображаются как коробочная паллета;
- у ворот должно быть понятно, кто привез паллету: РТК или комплектовщик.
...
- кликабельные маркеры потерь: красный кружок для пика, серый крестик для нулевой точки. Правый клик закрепляет окно расследования по минуте, где видно ресурсы, место, root-cause и потерянные минуты.
...
- Накопление перед воротами является отдельным временным окном: после `PALLET_STAGED_TO_DOCK` поддон должен оставаться в зоне накопления не менее `--dock-accumulation-minutes`, затем отгрузка/погрузка занимает `--dock-shipping-minutes`.
- При начале `PALLET_LOADING_STARTED` digital twin должен показывать машину у ворот, чтобы оператор видел различие между накоплением перед воротами и фактической погрузкой.
...
- Ресурсы на карте используют 4-направленные raster sprite sheets: ричтрак из `reachtruck_4dir_sprite_pack`, комплектовщик с электротележкой из `picker_4dir_sprite_pack`; canvas выбирает кадр по направлению движения.
- Шлейф движения стал затухающим пунктиром со стрелками; время жизни шлейфа задается в настройках модели и не может превышать 20 минут.
...
- Проходы отбора визуализируются как отдельные яркие транспортные полосы; pick-face адреса рисуются двумя сторонами вокруг прохода с Z-порядком `left/right` по парам слотов.
- Pick-face ячейки визуализируют наполненность: высота и цвет столбика меняются по расчетному остатку в текущую минуту.
- Изометрическая сцена усилена под референс control-tower: крупнее показаны подписи проходов, gate labels и зоны накопления перед воротами.
- График потерь минут имеет кликабельные красные пики и серые нулевые маркеры; если курсор находится на нулевом маркере, выбор не должен перескакивать на удаленный пик потерь.
