...
| 10 | Видеть метрики качества плана и forecast | history/forecast endpoints; forecast без тяжелой repeated view scan; planner history bounded by default `limit=25` and explicit `limit` clamp `1..500` | `test_sprint10_functional.py`, `transport_sprint10_load_test.py`, `transport_sprint10_ui_smoke.cjs` | Fresh hardening passed: `17 passed`; history load p95 `259.2 ms`; fresh slow SQL after fix `[]` |
...
| Финальный daily VRP example | Дневная заявка `2026-05-24` автоматически распределяется по рейсам с учетом паллетной вместимости, окон приемки магазина, routing provider mode и прогноза доступности seed-флота по 4 рейса/машину в день. | `test_final_vrp_daily_acceptance.py`; включен в PowerShell/Python release gate |
| Wave 3: Business Factor Trace | Третья волна тестирования доказывает, что бизнес-факторы не просто есть в моделях, а проходят end-to-end через source SQL/API/DTO/алгоритм/ответ/состояние. Правила v2 приняты в Wave 3: проверять значения, distinct/non-default, cross-source equality, business invariants, cleanup for flows, and payload contract under load. Coverage lock покрывает всю TMS-2 функциональность: dispatcher, UX 30-95, fleet/admin, MAP/VRP, ARM/Gantt, billing, users/rights, driver mobile, GPS/geofence, KPI and tariffs. Новый endpoint без Wave 3 владельца теперь валит gate. | `test_wave3_business_factor_trace.py`; `test_wave3_full_system_coverage.py`; `transport_wave3_business_factor_load_test.py`; functional включен в PowerShell/Python release gate, load runner включается флагом Wave 3 |
...
Current strict local result on 2026-05-29 after Phase 3 migrations (055-062), final daily VRP gate, v2-strengthened full-system Wave 3 Business Factor Trace, and planner-history hardening were added: `scripts\tms2-release-gate.ps1 -SeedDate 2026-05-24 -IncludeSprint60To95 -IncludeUiSmoke -IncludeLoadSmoke -IncludeWave3` -> core **`446 passed`**, Sprint 60-95 **`234 passed`**, all load/UI/NFR/Wave 3 gates passed, **no pytest skips**. Fresh slow SQL review after the full gate from `from_log_id=1153` returned `[]` above `500 ms`.
...
- combined strict release gate runner: `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\tms2-release-gate.ps1 -SeedDate 2026-05-24 -IncludeSprint60To95 -IncludeUiSmoke -IncludeLoadSmoke -IncludeWave3`;
