...
Статус: реализован runtime-инкремент для `IMMEDIATE`, `WAIT_MINIMAX`, ручного/API Minimax-релиза, выбора паллеты-источника, hard reservation и автоматического Minimax-триггера после факта `CASE_PICK`.
...
2. Для каждой строки заранее подбирается источник и создается `HARD` reservation на source pallet.
