...
- dynamic/generic pick face не является второй заранее заполненной ячейкой отбора для SKU;
- плановое pre-wave пополнение не должно занимать dynamic/generic ячейки;
- если на складе есть активная ячейка отбора, которой не назначен жестко приколоченный SKU, и она свободна по остатку, hard reservations и активным warehouse tasks, то система может выпустить ричтрак-задачу на спуск туда только при urgent-событии;
- urgent-событие возникает, когда fixed pick face не обеспечивает текущий отбор или predictive lead-time показывает, что fixed pick face не успеет пополниться вовремя;
...
- если комплектовщик пришел к пустой fixed pick face, но тот же SKU уже есть в dynamic/generic ячейке дальше по маршруту, WMS может переставить текущую строку отбора на эту ячейку;
...
## 7.2. Оптимизация Layout На Сутки

Перед операционным днем WMS должна уметь рассчитать рекомендации по layout ячеек отбора на основе множества заказов, например на следующие сутки и 10 волн.

Вход:

- заказы на плановый горизонт;
- прогноз волн и маршрутов;
- ATO/SKU, которые будут нуждаться в пополнении;
- текущие fixed pick-face назначения;
- список dynamic/overflow ячеек;
- ресурсная мощность ричтраков и комплектовщиков.

Выход:

- список SKU, которым не хватает текущей емкости pick face;
- рекомендации переназначить или добавить duplicate pick-face;
- рекомендации по доле dynamic/overflow ячеек;
- прогноз количества urgent-пополнений;
- предупреждения, где fixed layout создаст ожидание комплектовщика или перегруз ричтраков.

Базовое правило для тестового прогона: 10% ячеек отбора выделены как dynamic/overflow и расположены в конце аллей. В production это административная настройка layout, а не жесткий алгоритм.
