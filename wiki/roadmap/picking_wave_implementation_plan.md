...
### 2.3.1. Волна Как Единственный Управляющий Документ Исполнения

Для задач ричтрака в контуре комплектации управляющим документом является только волна.

Заказы на отгрузку и производственные заказы могут порождать волну или входить в нее как контекст, но физические задачи водителя должны ссылаться на волну:
...
- пополнение pick face под покоробочную комплектацию создается внутри волны;
- спуск паллета в грузовую зону создается как задача волны;
- самостоятельный `SOURCE_DOC_TYPE = SHIPMENT` не используется как первичный источник reachtruck task;
- производственные волны в целевой модели должны использовать тот же принцип, но с отдельным типом волны или `PRODUCTION_WAVE`.
...
### Этап 7.1. Case-Pick Replenishment Under Wave

Цель: реализовать отдельный контур пополнения ячеек отбора для покоробочной комплектации.

Результат:

- `IMMEDIATE` strategy: задача пополнения выпускается при первой потребности волны;
- `MINIMAX` strategy: доменная строка создается при расчете волны, но водительская задача выпускается только после снижения фактического остатка ниже порога;
- поддерживаются полный паллет, полупаллет и добивка pick face до полной емкости по объему;
- `RRL_PICK_WAVE_REPLENISH_TASK` хранит доменную строку;
- `RRL_WAREHOUSE_TASK` хранит физическое исполнение водителем.
