...
- [requirements/large_warehouse_route_consumption_readiness_2026_05_23.md](requirements/large_warehouse_route_consumption_readiness_2026_05_23.md): first post-release route-consumption sprint for the large warehouse map: adds a read-only picking readiness API that checks whether published topology/route rows are consumable by wave/case-pick through pick-face and articul bindings.
- [requirements/large_warehouse_route_consumption_materialization_2026_05_23.md](requirements/large_warehouse_route_consumption_materialization_2026_05_23.md): second post-release route-consumption sprint for the large warehouse map: materializes published pick-route cells into `RRL_PICK_FACE`, supports explicit articul bindings, proves duplicate-safe retry on `WARE_ID=1`, and leaves only real SKU binding as the next blocker.
- [requirements/large_warehouse_route_binding_loader_2026_05_23.md](requirements/large_warehouse_route_binding_loader_2026_05_23.md): third post-release route-consumption sprint for the large warehouse map: adds a safe CSV/JSON binding loader that defaults to dry-run and applies explicit SKU-to-pick-face mappings through the materialize API.
- [requirements/large_warehouse_map_excel_canvas_actions_tz.md](requirements/large_warehouse_map_excel_canvas_actions_tz.md): Russian technical assignment for expanding warehouse map canvas actions by Excel analogy: every working function must have a left-menu action and a context-menu tree duplicate, `Сохранить канвас` must exist in both places, Excel functions are mapped to needed canvas-map analogs with proposed icons and priorities, the first implementation checkpoint adds an Excel-like `Главная` ribbon as a third fast entry point, `Excel Actions 2` adds parity for editing/filters/navigation/validation, and `Excel Actions 3` compacts the ribbon/context menu while adding template/layout and multi-pattern route actions.
