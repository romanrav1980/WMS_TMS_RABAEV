...
- мягкая потребность сырья и жесткое WMS-резервирование сырья;
...
2. WMS создает жесткий резерв только на конкретные сырьевые партии/паллеты/количества. Мягкая потребность без партии и паллеты остается плановой потребностью MES и не занимает WMS-остаток.

Резервы целевого контура должны храниться в одной общей таблице `RRL_STOCK_RESERVATION`:

- `RESERVATION_KIND = SOFT` - артикул и количество, физические поля склада/ячейки/партии/паллета пустые;
- `RESERVATION_KIND = HARD` - артикул, количество, склад, ячейка, партия/производственная партия и паллет.

Только `HARD` участвует в расчете свободного остатка. `SOFT` используется для планирования, дефицита и перехода к `HARD`, когда выбран конкретный WMS-остаток.
