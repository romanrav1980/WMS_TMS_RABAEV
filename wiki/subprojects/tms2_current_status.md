...
- Sprint 10 planner history is now bounded and concurrency-hardened: default `limit=25`, explicit `limit` is clamped to `1..500`, rows are newest-first, and the service cache lookup/fetch/store is protected by one lock to avoid a cold-cache stampede.
...
## Final Sprint checkpoint — 2026-05-29

- Final Sprint scope accepted: strict release acceptance, Oracle/prod readiness, controlled Sprint 8 mutating VRP apply, routing/NFR proof, and slow SQL review. No new product features outside TMS-2 were taken.
- Added final daily VRP acceptance gate: `tests/transport/test_final_vrp_daily_acceptance.py`. It proves the `2026-05-24` daily ST set exists, the Dobrotseny seed fleet `9201..9215` can cover four dispatch slots per vehicle by pallet capacity and `/vehicles/available` forecast, routing provider mode is known, and `/planner/solve` returns non-empty routes with capacity respected, zero unassigned STs, and visible store time windows.
- Added the third testing wave, **Wave 3: Business Factor Trace**. Functional/E2E gate `tests/transport/test_wave3_business_factor_trace.py` now uses the v2 data-fidelity rules: value checks, distinct/non-default checks, cross-source equality (`available-sts` ↔ `planner/orders` ↔ VRP stops), business invariants over all stops, and a cleanup-bound flow (`available-sts` → temp route → `task/sts`). Full-system coverage lock `tests/transport/test_wave3_full_system_coverage.py` imports real FastAPI routers and requires every TMS-2 HTTP endpoint to have a Wave 3 business-process owner, factors, source/state evidence, functional gate, and load/UI/E2E evidence. Load gate `tests/transport/transport_wave3_business_factor_load_test.py` checks payload contracts and non-default values for every parallel response, including `/available-sts`, `/planner/orders`, availability, routing, and solve.
- Fixed planner time-window propagation: `get_planner_orders()` now reads `ZONE_TIME_PLAN_IN/OUT` and address `ORD`; `solve_vrp()` converts Oracle time values into minutes from the 06:00 shift start. When legacy ST rows do not carry windows, the planner uses a deterministic fallback by address `ORD` (`07:00`, `10:00`, `13:00`, `16:00` windows) so the daily acceptance example remains testable on the Dobrotseny seed.
- Hardened mutating Sprint 8 evidence: `apply_vrp_plan()` now returns `created_task_ids`; optional mutating Sprint 8 test verifies created routes have assigned STs and then cancels the created tasks to release STs.
- Verification: `test_final_vrp_daily_acceptance.py` -> `3 passed`; Wave 3 factor trace + full-system coverage -> `14 passed`; Wave 3 load gate passed (`available-sts` p95 `71.7 ms`, `planner/orders` p95 `316.3 ms`, availability p95 `117.8 ms`, routing status p95 `4409.8 ms`, solve p95 `1759.9 ms`); `test_final_vrp_daily_acceptance.py + test_sprint8_functional.py` -> `26 passed`; mutating Sprint 8 gate with `TMS_RUN_MUTATING_VRP_APPLY=1` and seed `2026-05-24` -> `23 passed`; post-cleanup API check found `active_vrp_auto_tasks=0`.
- Strict release gate after v2-strengthened Wave 3: `scripts\tms2-release-gate.ps1 -SeedDate 2026-05-24` -> `444 passed`, no pytest skips. Earlier full gate with `-IncludeSprint60To95 -IncludeUiSmoke -IncludeLoadSmoke` -> core `430 passed`, Sprint 60-95 `234 passed`, all load scripts passed, grouped UI smoke passed, and 2000-row NFR smoke passed (`firstRenderMs=894`, rows `31/43/33`).
- Routing/NFR evidence: `scripts\tms2-routing-smoke.ps1` passed with OSRM route HTTP 200 and Valhalla `/status` HTTP 200; `/routing/status` reports `active_provider=osrm`, `osrm_available=true`, `valhalla_available=true`, `haversine_available=true`. Separate `transport_table_2000_nfr_smoke.cjs` passed with `firstRenderMs=807`, rows `31/43/33`.
- Oracle/prod readiness evidence: production checklist corrected to the actual migration file sequence `042,043,051,052,053,054,055,056,058,060,062`; Dobrotseny `046/047` and planner template fixture `055` are dev/acceptance fixtures only and must not be applied to prod. Local rights API proves `GLOBAL_ADMIN`, `LOGIST`, `ST_PRIEMO_SD`, and transport rights are visible; staging/prod DSNs are not configured in the current workspace, so real staging/prod applied-migration verification remains a DBA/Tech Lead gate.
- Planner history slow-SQL gap was closed before final sign-off: `/planner/history` is bounded by default to 25 rows, supports explicit `limit`, uses newest-first `FETCH FIRST`, and avoids parallel cold-cache duplicate DB reads. Targeted Sprint 10 load after the fix passed with history p95 `259.2 ms`; fresh slow SQL from `from_log_id=1153` returned `[]`.
- Final full sign-off gate after the fix: `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\tms2-release-gate.ps1 -SeedDate 2026-05-24 -IncludeSprint60To95 -IncludeUiSmoke -IncludeLoadSmoke -IncludeWave3` passed. Evidence: routing smoke OSRM/Valhalla passed; core `446 passed`, no pytest skips; Sprint 60-95 `234 passed`; all Sprint 60-95 load scripts passed; Wave 3 load passed (`available-sts` p95 `62.3 ms`, `planner/orders` p95 `283.4 ms`, availability p95 `148.4 ms`, routing status p95 `4268.2 ms`, solve p95 `1339.4 ms`); UI smoke passed; 2000-row NFR smoke passed with `firstRenderMs=763` and bounded rows `31/43/33`.
- Mandatory slow SQL review after the final full sign-off gate from `from_log_id=1153`: `[]`. Decision: no fresh transport SQL above `500 ms`; old `/planner/history` rows with 100+ row counts are pre-fix evidence and no longer release blockers.
- Neighbor-agent v2 acceptance runbook was executed on the current local environment against seed `2026-05-24`: dependencies `pytest-html` and Playwright Chromium installed, API run without `--reload` for NFR stability, screenshots captured under `reports/screenshots/`, and `reports/FULL_REPORT.html` generated.
- v2 verification result after the final handoff pass: `tests/transport/v2/` -> `130 passed, 7 skipped`, no failures. Group order followed `tests/transport/v2/AGENT_RUNBOOK.md`; group reports and the aggregate `reports/FULL_REPORT.html` were regenerated, and screenshots were captured under `reports/screenshots/`. The skips are data/precondition skips (for example no `TW_STRICT=1` seed rows and billed-route lifecycle branches not available in the current fixture), not HTTP/contract failures.
