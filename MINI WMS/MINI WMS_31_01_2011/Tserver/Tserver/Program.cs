            ...
            switch (F_Request1.function_name)
            {
                ...
                case "LOT_CHECK_PASSED":
                    ...
                    catch (Exception ex)
                    {
                        ...
                        r.add_value("REASON", ex.Message);
                        summary = summary + r.encode();
                        Console.WriteLine("Error F4:"+ex.Message);
                    }
                    #endregion
...
" ORDER BY ТранспортныеЗаданияСтроки.ДатаТЗ, ТранспортныеЗадания.Экспедитор; ";
    ...
    class Program
    {
        ...
        {
            IP_listen = "192.168.208.205";
            IP_listen = "192.168.208.200"; PORT_p = 20000;
           // IP_listen = "192.168.208.205";
        ...
        if (args.Length > 0)
        {
            ...
            Console_WriteLine("Сервер приложений стартовал. Адрес прослушивания = " + IP_listen + " ПОРТ = " + PORT_p.ToString() + " Склад=  " + ware_id.ToString() + "  \n Версия= " + VERSION1);
            while (true)
            {
                try
                {
