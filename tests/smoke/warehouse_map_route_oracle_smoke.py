...
WARE_ID = -41224
CREATED_BY = "SMOKE_022"
ROOT_DIR = ROOT / "runtime" / "test-evidence" / "warehouse-map-sprint22-drafts"
...
def main() -> None:
    ...
    try:
        gateway.execute(
            ...
            {"ware_id": WARE_ID, "name": "SMOKE 022 MAP ROUTE", "prefix": "S22"},
        )
        draft = service.create_draft(
            WarehouseMapDraftCreateRequest(
                draft_name="Smoke 022 route draft",
        ...
        route = service.build_route(
            ...
            WarehouseMapDraftRouteBuildRequest(
                ...
                route_code="SMOKE-022-PICK",
                route_name="Smoke 022 route",
        ...
        saved = service.save_to_db(
            ...
            WarehouseMapDraftSaveToDbRequest(
                ware_id=WARE_ID,
                canvas_code="SMOKE-022-CANVAS",
                canvas_name="Smoke 022 canvas",
                camera_code="SMOKE-022-MAIN",
                camera_name="Smoke 022 main camera",
                updated_by=CREATED_BY,
            ),
            CREATED_BY,
        )
        projection = service.save_projection_to_topology(
            ...
            WarehouseMapDraftProjectionSaveRequest(
                ware_id=WARE_ID,
                topology_code="SMOKE-022-TOPO",
                topology_name="Smoke 022 topology",
                updated_by=CREATED_BY,
            ),
            CREATED_BY,
        )
        ...
        direct = direct_counts(gateway, int(route_save["pick_route_id"]))

        assert route["route_row_count"] == 3, route
        assert route_save["route_row_count"] == 3, route_save
        assert route_save["oracle_validation"]["valid"], route_save
        assert direct["route_row_count"] == 3, direct
        assert direct["physical_cell_rows"] == 2, direct
        assert direct["pick_slot_rows"] == 1, direct
...
def direct_counts(gateway: OracleGateway, pick_route_id: int) -> dict[str, int]:
    rows = gateway.fetch_all(
        """
        select 'route_row_count' key, count(*) value
          from RRL_PICK_ROUTE_CELL
         where PICK_ROUTE_ID = :pick_route_id
           and ACTIVE = 1
        union all
        select 'physical_cell_rows', count(*)
          from RRL_PICK_ROUTE_CELL
         where PICK_ROUTE_ID = :pick_route_id
           and TOPOLOGY_CELL_ID is not null
           and ACTIVE = 1
        union all
        select 'pick_slot_rows', count(*)
         ...
         where rc.PICK_ROUTE_ID = :pick_route_id
           ...
           and rc.ACTIVE = 1
        union all
        select 'storage_slot_rows', count(*)
          from RRL_PICK_ROUTE_CELL rc
          join RRL_TOPOLOGY_CELL_SLOT s
            on s.CELL_SLOT_ID = rc.CELL_SLOT_ID
         where rc.PICK_ROUTE_ID = :pick_route_id
