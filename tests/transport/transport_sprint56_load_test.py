"""Windows-safe load check for Sprint 56 (print route sheet).

Printing is client-side HTML generation. This runner measures the read
endpoints needed to select a trip and load its ST composition. No mutation is
performed.
"""
...
def discover_task_id() -> int | None:
    ...
    for task in response.json():
        if int(task.get("ST_COUNT") or 0) > 0:
            return int(task["ID"])
    return int(response.json()[0]["ID"]) if response.json() else None


def main() -> int:
    task_id = discover_task_id()
    cases = [
        EndpointCase("GET /tasks print context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, include_readiness='true')}", 700, 30, 5),
        EndpointCase("GET /available-sts print context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 30, 5),
    ]
    if task_id is not None:
        cases.insert(0, EndpointCase("GET /tasks/{id}/sts print context", f"/api/admin/transport/tasks/{task_id}/sts", 400, 30, 5))
    for case in cases:
