"""Windows-safe load check for Sprint 51 (sticky selection bar).

Selection is frontend state, so the safe backend context is reading the ST list
and task list used before selecting rows. No mutating assignment calls are made.
"""
...
def main() -> int:
    cases = [
        EndpointCase(
            "GET /available-sts selection-bar context",
        ...
        EndpointCase(
            "GET /tasks selection-bar context",
