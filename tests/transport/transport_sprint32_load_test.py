"""Windows-safe load check for Sprint 32 (overload warning data endpoints)."""
...
def sample_task_id() -> int:
    ...
    tasks = response.json()
    if not tasks:
        raise RuntimeError("No task available for Sprint 32 load gate")
...
def main() -> int:
    ...
    cases = [
        ...
        EndpointCase("GET /vehicles", "GET", "/api/admin/transport/vehicles", 200, 40, 5),
    ]
    for case in cases:
        requests.request(case.method, f"{BASE_URL}{case.path}", auth=AUTH, timeout=20)
    ok = True
    for case in cases:
