"""Windows-safe load check for Sprint 26 (single billing order XLSX export)."""
...
def sample_order_id() -> int:
    ...
    if not orders:
        raise RuntimeError("No billing order available for Sprint 26 load gate")
...
def main() -> int:
    ...
    for path in (
        ...
        f"/api/admin/transport/billing/orders/{order_id}/export.xlsx",
    ):
        requests.get(f"{BASE_URL}{path}", auth=AUTH, timeout=20)
    cases = [
        EndpointCase("GET /billing/orders", "GET", "/api/admin/transport/billing/orders", 300, 40, 5),
        EndpointCase("GET /billing/orders/{id}/tasks", "GET", f"/api/admin/transport/billing/orders/{order_id}/tasks", 300, 40, 5),
        EndpointCase("GET /billing/orders/{id}/export.xlsx", "GET", f"/api/admin/transport/billing/orders/{order_id}/export.xlsx", 500, 30, 4),
    ]
    ok = True
    for case in cases:
