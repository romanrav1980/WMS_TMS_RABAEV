"""Windows-safe load check for Sprint 41 (routes status filter source).

The route status filter is client-side. This runner measures the task list
reads used by the routes tab; it does not mutate Oracle data.
"""
...
def main() -> int:
    cases = [
        EndpointCase(
            "GET /tasks routes all statuses",
            f"/api/admin/transport/tasks?{qs(shipment_date=PLAN_DATE, include_readiness='true')}",
            700,
            40,
            5,
        ),
        EndpointCase(
            "GET /tasks routes date range",
            f"/api/admin/transport/tasks?{qs(shipment_date=PLAN_DATE, date_to='2026-05-31', include_readiness='true')}",
            700,
            40,
            5,
        ),
        EndpointCase(
            "GET /tasks routes no payments",
            f"/api/admin/transport/tasks?{qs(shipment_date=PLAN_DATE, no_payments_only='true')}",
            700,
            30,
            5,
        ),
