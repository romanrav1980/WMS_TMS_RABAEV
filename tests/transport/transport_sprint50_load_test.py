"""Windows-safe load check for Sprint 50 (ready-ST highlight).

The highlight uses VERIFY_PERC from AvailableSt rows. This runner measures the
read endpoints that provide verify data without mutating Oracle.
"""
...
def main() -> int:
    cases = [
        EndpointCase(
            "GET /available-sts assembled ready-highlight",
            f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, assembled_only='true')}",
            600,
            40,
            5,
        ),
        EndpointCase(
            "GET /available-sts all ready-highlight",
