"""Windows-safe load check for Sprint 52 (sortable ST table).

Sorting is client-side, so this runner measures the available-ST and task reads
that feed the table. No mutation is performed.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts sort context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 40, 5),
        EndpointCase("GET /available-sts warehouse sort context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, ware_id=9201)}", 600, 30, 5),
        EndpointCase("GET /tasks sort context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, include_readiness='true')}", 700, 30, 5),
    ]
    for case in cases:
        request_once(case)
    ok = True
