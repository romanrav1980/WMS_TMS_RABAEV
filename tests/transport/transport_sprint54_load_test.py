"""Windows-safe load check for Sprint 54 (selected-ST CSV export).

CSV generation is client-side, so this runner measures the read endpoints that
feed selection and export. No mutation is performed.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts CSV export context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 40, 5),
        EndpointCase("GET /available-sts selected warehouse CSV context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, ware_id=9201)}", 600, 30, 5),
        EndpointCase("GET /tasks CSV export context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, include_readiness='true')}", 700, 30, 5),
    ]
    for case in cases:
