"""Windows-safe load check for Sprint 45 (goto-trip button).

Goto-trip may fetch a single task when it is not in the loaded list. This
runner uses read-only paths, including a safe missing-task 404 probe.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts all for goto", f"/api/admin/transport/available-sts?{qs(stdate=PLAN_DATE, unassigned_only='false')}", 500, 40, 5),
        EndpointCase("GET /tasks routes tab", f"/api/admin/transport/tasks?{qs(shipment_date=PLAN_DATE, include_readiness='true')}", 700, 40, 5),
        EndpointCase("GET /tasks/{id} missing", f"/api/admin/transport/tasks/{MISSING_TASK_ID}", 400, 30, 5, (404,)),
    ]
    for case in cases:
        request_once(case)
    ok = True
