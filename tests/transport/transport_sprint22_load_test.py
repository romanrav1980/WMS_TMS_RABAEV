"""Windows-safe load check for Sprint 22 (billing companies + NUM_PLAT)."""
...
def main() -> int:
    cases = [
        EndpointCase("GET /billing/companies", "GET", "/api/admin/transport/billing/companies", 200, 50, 6),
        EndpointCase("GET /billing/orders", "GET", "/api/admin/transport/billing/orders", 300, 50, 6),
        EndpointCase("GET /billing/orders?closed=0", "GET", "/api/admin/transport/billing/orders?closed=0", 300, 50, 6),
    ]
    ok = True
    for case in cases:
