"""Windows-safe load check for Sprint 21 (billing RBAC gates)."""
...
def main() -> int:
    cases = [
        ...
        EndpointCase("GET /billing/orders?closed=0", "GET", "/api/admin/transport/billing/orders?closed=0&payed=0", 300, 40, 5),
        EndpointCase(
            "POST /billing/orders",
            "POST",
            ...
            20,
            3,
            {"company": "ООО Нагрузка-21", "date_from": TODAY, "date_to": TODAY},
        ),
        EndpointCase(
            "POST /tasks/{id}/recalculate-price",
            "POST",
            "/api/admin/transport/tasks/1/recalculate-price",
            400,
            25,
            4,
            ok_statuses={200, 404, 409},
        ),
        EndpointCase(
            "PATCH /tasks/{id}/price",
            "PATCH",
            "/api/admin/transport/tasks/1/price",
            400,
            25,
            4,
            {"price": 1000},
