"""Windows-safe load check for Sprint 37 (auto-refresh polling endpoints)."""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts auto", "GET", f"/api/admin/transport/available-sts?stdate={PLAN_DATE}&unassigned_only=true", 400, 40, 10),
        EndpointCase("GET /tasks auto", "GET", f"/api/admin/transport/tasks?shipment_date={PLAN_DATE}&include_readiness=true", 400, 40, 10),
        EndpointCase("GET /clusters auto", "GET", f"/api/admin/transport/clusters?stdate={PLAN_DATE}", 300, 40, 10),
    ]
    for case in cases:
        request_once(case)
    ok = True
