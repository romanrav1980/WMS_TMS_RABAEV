"""
transport_sprint64_load_test.py — Load test for Sprint 64 (click-to-select not-ready).

Pure frontend — no new backend endpoints.
We test /available-sts (VERIFY_PERC used for selection logic).
...
class SelectNotReadyUser(HttpUser):
    ...
    @task(2)
    def load_available_sts_not_assembled(self):
        from datetime import date
        today = date.today().isoformat()
        self.client.get(
            f"/api/admin/transport/available-sts?stdate={today}&not_assembled_only=true",
            name="/available-sts?not_assembled",
        )
