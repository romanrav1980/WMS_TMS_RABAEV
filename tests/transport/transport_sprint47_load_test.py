"""Windows-safe load check for Sprint 47 (localStorage restore).

localStorage is browser-side; this runner measures the initial read endpoints
that fire after persisted dates/tabs are restored. It does not mutate Oracle.
"""
...
def main() -> int:
    cases = [
        EndpointCase(
            "GET /tasks localStorage restore",
            f"/api/admin/transport/tasks?{qs(shipment_date=RESTORED_DATE, include_readiness='true')}",
            700,
            40,
            5,
        ),
        EndpointCase(
            "GET /available-sts localStorage restore",
            f"/api/admin/transport/available-sts?{qs(stdate=RESTORED_DATE)}",
            600,
            40,
            5,
        ),
        EndpointCase("GET /clusters localStorage restore", f"/api/admin/transport/clusters?{qs(stdate=RESTORED_DATE)}", 500, 30, 5),
    ]
    for case in cases:
        request_once(case)
    ok = True
