"""Windows-safe load check for Sprint 48 (Today button).

The button itself is browser state; pressing it reloads the same read endpoints
as manual date changes. This runner measures those reads without mutating
Oracle data and without requiring Locust.
"""
...
def main() -> int:
    cases = [
        EndpointCase(
            "GET /tasks today reset",
            f"/api/admin/transport/tasks?{qs(shipment_date=TODAY, include_readiness='true')}",
            700,
            40,
            5,
        ),
        EndpointCase(
            "GET /available-sts today reset",
            f"/api/admin/transport/available-sts?{qs(stdate=TODAY)}",
            600,
            40,
            5,
        ),
