"""Windows-safe load check for Sprint 59 (quick search in routes tab).

Search is client-side over loaded tasks. This runner measures route-list reads
with and without common backend filters. No mutation is performed.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /tasks route search context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, include_readiness='true')}", 700, 40, 5),
        EndpointCase("GET /tasks route search car context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, transport='Е715')}", 700, 30, 5),
        EndpointCase("GET /tasks route search task-id context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, task_id=1001)}", 700, 20, 4),
    ]
    for case in cases:
