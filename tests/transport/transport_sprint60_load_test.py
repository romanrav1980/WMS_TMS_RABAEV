"""
transport_sprint60_load_test.py — Load test for Sprint 60 (STs table pagination).

Pure frontend — no new backend endpoints.
We test /available-sts (data loaded before pagination).
...
class StsPaginationUser(HttpUser):
    ...
    def load_available_sts_filtered(self):
        ...
        today = date.today().isoformat()
        self.client.get(
            f"/api/admin/transport/available-sts?stdate={today}&transport_type=10",
            name="/available-sts?transport_type=",
        )
