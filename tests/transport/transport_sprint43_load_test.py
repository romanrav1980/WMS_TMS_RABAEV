"""Windows-safe load check for Sprint 43 (sortable trips table source).

Sorting is client-side. This runner measures the task list reads used by the
tasks tab and one adjacent available-ST read; it does not mutate Oracle data.
"""
...
def main() -> int:
    cases = [
        ...
        EndpointCase(
            "GET /tasks sort date range",
            f"/api/admin/transport/tasks?{qs(shipment_date='2026-05-01', date_to='2026-05-31', include_readiness='true')}",
            700,
            40,
            5,
        ),
