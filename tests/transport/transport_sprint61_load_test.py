"""
transport_sprint61_load_test.py — Load test for Sprint 61 (warehouse quick-filter).

Pure frontend filter — no new backend endpoints.
We test /available-sts (data loaded before filtering).
...
class WareFilterUser(HttpUser):
    ...
    @task(2)
    def load_available_sts_ware(self):
        from datetime import date
        today = date.today().isoformat()
        self.client.get(
