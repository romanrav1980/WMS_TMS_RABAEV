"""Windows-safe load check for Sprint 53 (collapsible filter panel).

The feature is client-side, so this runner measures the read endpoints that
feed the filter panel context. No mutation is performed.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts filter panel default", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 40, 5),
        EndpointCase("GET /available-sts filter panel addr", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, addr_mask='Пермь')}", 600, 30, 5),
        EndpointCase("GET /tasks filter panel context", f"/api/admin/transport/tasks?{qs(shipment_date=SEED_DATE, include_readiness='true')}", 700, 30, 5),
    ]
    for case in cases:
