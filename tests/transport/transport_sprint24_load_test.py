"""Windows-safe load check for Sprint 24 (planner DnD regressions)."""
...
def main() -> int:
    warmups = [
        ...
        "/api/admin/transport/planner/history?date_from=2026-05-25&date_to=2026-05-25",
        "/api/admin/transport/routing/status",
    ]
    for path in warmups:
        requests.get(f"{BASE_URL}{path}", auth=AUTH, timeout=20)
    cases = [
        EndpointCase("GET /planner/metrics", "GET", "/api/admin/transport/planner/metrics", 200, 50, 6),
        EndpointCase("GET /planner/history", "GET", "/api/admin/transport/planner/history?date_from=2026-05-25&date_to=2026-05-25", 300, 50, 6),
        EndpointCase("GET /routing/status", "GET", "/api/admin/transport/routing/status", 200, 40, 5),
    ]
    ok = True
    for case in cases:
