"""Windows-safe load check for Sprint 58 (expand/collapse all clusters)."""
...
def main() -> int:
    cases = [
        EndpointCase("GET /clusters expand-all context", f"/api/admin/transport/clusters?{qs(stdate=SEED_DATE)}", 500, 40, 5),
        EndpointCase("GET /available-sts cluster context", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 30, 5),
    ]
    for case in cases:
