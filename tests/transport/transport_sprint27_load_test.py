"""Windows-safe load check for Sprint 27 (billing registry XLSX export)."""
...
def sample_order_id() -> int:
    ...
    if not orders:
        raise RuntimeError("No billing order available for Sprint 27 load gate")
...
def main() -> int:
    ...
    cases = [
        ...
        EndpointCase("GET /billing/orders/export.xlsx", "GET", "/api/admin/transport/billing/orders/export.xlsx", 800, 25, 4),
        EndpointCase("GET /billing/orders/{id}/export.xlsx", "GET", f"/api/admin/transport/billing/orders/{order_id}/export.xlsx", 500, 25, 4),
    ]
    ok = True
    for case in cases:
