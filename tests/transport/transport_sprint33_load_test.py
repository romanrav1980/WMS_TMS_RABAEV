"""Windows-safe load check for Sprint 33 (routes brief mode data endpoint)."""
...
def main() -> int:
    case = EndpointCase("GET /tasks", "GET", f"/api/admin/transport/tasks?shipment_date={PLAN_DATE}", 300, 40, 5)
    requests.request(case.method, f"{BASE_URL}{case.path}", auth=AUTH, timeout=20)
    return 0 if run_case(case) else 1
