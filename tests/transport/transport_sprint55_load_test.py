"""Windows-safe load check for Sprint 55 (all-visible ST totals).

Totals are client-side derived values over loaded STs. This runner measures the
read endpoints behind that view and performs no mutation.
"""
...
def main() -> int:
    cases = [
        EndpointCase("GET /available-sts visible totals all", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE)}", 600, 40, 5),
        EndpointCase("GET /available-sts visible totals warehouse", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, ware_id=9201)}", 600, 30, 5),
        EndpointCase("GET /available-sts visible totals unassigned", f"/api/admin/transport/available-sts?{qs(stdate=SEED_DATE, unassigned_only='true')}", 600, 30, 5),
    ]
    for case in cases:
